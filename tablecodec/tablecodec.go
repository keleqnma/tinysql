...
func DecodeRecordKey(key kv.Key) (tableID int64, handle int64, err error) {
	...
	if !key.HasPrefix(tablePrefix) {
		return 0, 0, errInvalidIndexKey.GenWithStack("invalid record key without tablePrefix- %q", k)
	}
	...
	if !key.HasPrefix(recordPrefixSep) {
		return 0, 0, errInvalidIndexKey.GenWithStack("invalid record key without recordPrefixSep- %q", k)
	}
	...
	/*
		if key != nil {
			return 0, 0, errInvalidIndexKey.GenWithStack("invalid record key - %q", k)
		}
	*/
